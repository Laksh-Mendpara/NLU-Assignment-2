...
from inference.semantic_analysis import build_analysis_exclusions
    ...
    if os.path.exists(summary_path):
        try:
            ...
            for architecture in ("cbow", "skipgram"):
                ...
                if not candidates:
                    continue
                # Prefer the run with the strongest validation behavior for each architecture.
...
def build_word_list(artifact: dict, max_words: int) -> list[str]:
    ...
    excluded_tokens = build_analysis_exclusions(artifact)
    selected = [word for word in focus_words if word in tokens]

    # Start from a few anchor words, then expand with neighbors and common tokens.
    for word in ("research", "student", "academic"):
        for neighbor, _ in nearest_neighbors(artifact, word, topn=10, exclude_tokens=excluded_tokens):
...
def project_vectors(vectors: np.ndarray, method: str) -> np.ndarray:
    ...
    # For t-SNE, first shrink the space with PCA so it is more stable.
...
def plot_embeddings(model_path: str, output_dir: str, method: str, max_words: int) -> None:
    ...
    for index, word in enumerate(words_to_plot):
        ...
        # Highlight a small set of important academic words so the plot is easier to read.
...
def main():
    ...
    # Plot the same selected words for each model so the figures stay comparable.
    for model_path in model_paths:
        if not os.path.exists(model_path):
            print(f"Skipping missing artifact: {model_path}")
            continue
        for method in methods:
            plot_embeddings(model_path, args.plots_dir, method, args.max_words)
