...
from scraper.metadata import build_metadata, classify_doc_type
...
class IITJCrawler:
    ...
    async def crawl(self):
        ...
        async with aiohttp.ClientSession(
            ...
            # Use a small worker pool instead of spawning one task per URL.
            workers = [
                asyncio.create_task(self._worker(session, i))
                for i in range(self.concurrency)
            ]

            # `frontier.join()` waits until every queued URL calls `task_done()`.
    ...
    async def _worker(self, session: aiohttp.ClientSession, worker_id: int):
        ...
        while True:
            ...
            try:
                ...
                # Mark visited before fetching so parallel workers do not repeat work.
    ...
    async def _process_html(self, url: str, session: aiohttp.ClientSession):
        ...
        # Extract page text first, but still discover links even if we later drop the page.
        extracted = extract_content(html, url)
        doc_type = classify_doc_type(url)
        content, keep_document, reason = sanitize_document(
            url=url,
        ...
        if not keep_document:
            logger.warning("discarding data: %s", url)
            logger.debug("discard reason for %s: %s", url, reason)
            return
        ...
        # Save only after the page passes filtering and dedup.
        meta = build_metadata(
            url=url,
            title=extracted["title"],
    ...
    async def get_manifest(self) -> list[dict]:
        ...
        manifest = []
        for directory, storage_kind in ((self._data_dir, "html"), (self._pdf_dir, "pdf")):
